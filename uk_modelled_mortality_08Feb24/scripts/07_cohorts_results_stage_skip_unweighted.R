############################
#CORRECTION: Unweighted incidence round results. Weighted variables not used when calculating mean across levels of age
###########################
...
w_mort_by_cancer = weighted_inc_mort_by_cancer %>%  
  ...
  summarize(
    screen_detected = mean(cfdna), #renamed from cfdna_incidence
    SoC_detected = mean(soc), #renamed from soc_incidence
    mortality_no_screening = mean(original_deaths), #renamed from original_deaths
    mortality_screening = mean(shifted_deaths), #renamed from shifted_deaths
    screen_detected_prev = mean(cfdna_prevalence), #as above for the renaming conventions
    SoC_detected_prev = mean(soc_prevalence),
    mortality_no_screening_prev = mean(original_deaths_prevalence),
    mortality_screening_prev = mean(shifted_deaths_prevalence)
  ) %>%
...
w_SS_by_cancer <- weighted_stage_shift_by_cancer %>%
  ...
  summarize(
    early_stage = mean(early_stage),
    late_stage = mean(late_stage),
    shifted_early = mean(shifted_early),
    shifted_any = mean(shifted_any),
    early_stage_prev = mean(early_stage_prev),
    late_stage_prev = mean(late_stage_prev),
    shifted_early_prev = mean(shifted_early_prev),
    shifted_any_prev = mean(shifted_any_prev),
    shifted_percent = mean(shifted_percent),
    shifted_percent_prev = mean(shifted_percent_prev),
    early_percent = mean(early_percent),
    early_percent_prev = mean(early_percent_prev),
    initial_late = mean(initial_late),
    initial_late_prev = mean(initial_late_prev)
  ) %>%
  ungroup()
