...
w_mort_by_cancer = weighted_inc_mort_by_cancer %>%  
  ...
  summarize(
    screen_detected = mean(weighted_cfdna), #renamed from cfdna_incidence
    SoC_detected = mean(weighted_soc), #renamed from soc_incidence
    mortality_no_screening = mean(weighted_original_deaths), #renamed from original_deaths
    mortality_screening = mean(weighted_shifted_deaths), #renamed from shifted_deaths
...
w_SS_by_cancer <- weighted_stage_shift_by_cancer %>%
  ...
  summarize(
    early_stage = mean(weighted_early_stage),
    late_stage = mean(weighted_late_stage),
    shifted_early = mean(weighted_shifted_early),
    shifted_any = mean(weighted_shifted_any),
    ...
    shifted_percent = mean(weighted_shifted_percent),
    shifted_percent_prev = mean(shifted_percent_prev),
    early_percent = mean(weighted_early_percent),
    early_percent_prev = mean(early_percent_prev),
    initial_late = mean(weighted_initial_late),
