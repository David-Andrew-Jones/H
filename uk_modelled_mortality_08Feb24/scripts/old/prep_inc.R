...
full_inc_plus_fcn = fixed_incidence_df %>% 
  ...
  mutate(Low = as.numeric(Low),
         Hi = as.numeric(Hi)) %>%
  mutate(Age=(Low+Hi+1)/2) %>% #middle of age band
  group_by(NCRAS_Draw, Stage) %>%
  summarise(
    zeta=list(approxfun(tAge,Rate,rule=2)), 
    zeta_2=list(approxfun(tAge,Pop,rule=2)),
    zeta_3=list(approxfun(tAge,Count,rule=2)),
  ) %>%
  ungroup()
