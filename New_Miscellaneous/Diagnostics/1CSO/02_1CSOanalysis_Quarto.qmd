...
format:
   docx: 
    output-file: "Impact of MCED screening on diagnostic demand - 70% uptake, 0.93 CSO accuracy, 56% PPV"
    output-ext:  "docx"
---
...
IntercepMod_detection_rate_SS <- df_MM_cancer_classes_detected %>% 
      summarise(total_TP = sum(rate_1M_SS)) %>%
      mutate(total_FP = (total_TP/GalleriPPV) * (1-GalleriPPV),
             detection_rate = sum(total_TP + total_FP)) 
