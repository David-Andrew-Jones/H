...
res_path_allcause_descriptives_8cancers <- "~/Miscellaneous/HDI/Final_results2/Net costs/Unadjusted/Descriptives_GRAIL_net_2025-04-14.xlsx"
...
map(.f = function(x){ x %>% mutate(across(where(is.character), ~ case_when(grepl(c("78-06"), .) ~ str_replace(., "78-06","‘measured 78-6 months before diagnosis’"), .default = .))) }) %>%
  map(.f = function(x){ x %>% mutate(across(where(is.character), ~ case_when(grepl(c("GP referral"), .) ~ str_replace(., "GP referral","GP routine referral"), .default = .))) }) %>%
    map(.f = function(x){ x %>% mutate(across(where(is.character), ~ case_when(grepl(c("IQR"), .) ~ str_replace(., "IQR", "Quartiles"), .default = .))) }) %>%
      map(.f = function(x){ x %>% mutate(across(where(is.character), ~ case_when(grepl(c("Year of diagnosis"), .) ~ str_replace(., "Year of diagnosis", "Year of diagnosis (registration date)"), .default = .))) }) %>%

map(.f = function(x){ x %>% mutate(across(where(is.character), ~ gsub("([[:digit:]])[.]([[:digit:]])",
       "\\1·\\2", .)))  })
...
#' Need to change the % column to exclude stage 0
l_allcause_descriptives_8cancers[[10]] <- l_allcause_descriptives_8cancers[[10]] %>%
  separate_wider_delim(cols = `1`:`Unknown/Missing`,
                       delim = " ",
                       names_sep = "_") %>%
  select(-`0`) %>%
  mutate(across(dplyr::contains("_1"), as.numeric   ) ) %>%
  mutate(Total = `1_1` + `2_1` + `3_1` + `4_1` + `Unknown/Missing_1`) %>%
  mutate(across(dplyr::contains("_1") , \(x) round((x/Total *100),1), .names = "%_{col}")) %>%
  mutate(`1_1` = paste0(`1_1`, " ", "(", `%_1_1` ,")"),
         `2_1` = paste0(`2_1`, " ", "(", `%_2_1` ,")"),
         `3_1` = paste0(`3_1`, " ", "(", `%_3_1` ,")"),
         `4_1` = paste0(`4_1`, " ", "(", `%_4_1` ,")"),
         `Unknown/Missing_1` = paste0(`Unknown/Missing_1`, " ", "(", `%_Unknown/Missing_1` ,")")) %>%
    select(site, `diagnosis year`, `1_1`, `2_1` , `3_1`, `4_1`,  `Unknown/Missing_1`) %>%
  rename(`1` = `1_1`,
         `2` = `2_1` ,
         `3` = `3_1`, 
         `4` = `4_1`, 
         `Unknown/Missing` = `Unknown/Missing_1`) %>%
...
df_stage_missing <- bind_rows(
  ...
  select(`site`, `diagnosis year`,`1`,`2`,`3`, `4` ,`Unknown/Missing`)
...
l_allcause_descriptives_lymphoma <- res_path_allcause_descriptives_lymphoma %>% 
        excel_sheets() %>% 
        set_names() %>%
...
# Add censored
res_path_annual_and_phase_Ncensored_8cancers <- "~/Miscellaneous/HDI/Final_results2/Nandcensored_2025-09-30.xlsx"

df_allcause_annual_and_phase_Ncensored_8cancers <- res_path_annual_and_phase_Ncensored_8cancers %>% 
        excel_sheets() %>% 
        set_names() %>%
        map(~ read_excel(path = res_path_annual_and_phase_Ncensored_8cancers, sheet = .x), .id = "Sheet") %>%
  bind_cols() %>%
  select(-`site...28`, -`stage_best...29`) %>%
  rename(`Cancer type` = `site...1`,
         `Stage` = `stage_best...2`)%>%
        mutate(`Cancer type` = case_when(grepl(c("lung"), `Cancer type`) ~ "Lung",
                                         grepl(c("colorectal"), `Cancer type`) ~ "Colorectal",
                                         grepl(c("head_neck"), `Cancer type`) ~ "Head and neck",
                                         grepl(c("liver"), `Cancer type`) ~ "Liver",
                                         grepl(c("lymphoma"), `Cancer type`) ~ "Lymphoma",
                                         grepl(c("oesophagus"), `Cancer type`) ~ "Oesophageal",
                                         grepl(c("ovary"), `Cancer type`) ~ "Ovarian",
                                         grepl(c("pancreas"), `Cancer type`) ~ "Pancreatic",
                                         .default = `Cancer type`),.before = 1)   %>%
  mutate(across(`N_diag`:`censored_ac_treat_5`, \(x) as.numeric(x))) %>%
        mutate(`0-1 years after diagnosis:died in period (%)` = round((`died_ac_treat_0_1`/`N_ac_treat_0_1`)*100, 2),
               `0-1 years after diagnosis:censored (%)` = round((`censored_ac_treat_0_1`/`N_ac_treat_0_1`)*100, 2),
               `1-2 years after diagnosis:died in period (%)` = round((`died_ac_treat_1_2`/`N_ac_treat_1_2`)*100, 2),
               `1-2 years after diagnosis:censored (%)` = round((`censored_ac_treat_1_2`/`N_ac_treat_1_2`)*100, 2),
               `2-3 years after diagnosis:died in period (%)` = round((`died_ac_treat_2_3`/`N_ac_treat_2_3`)*100, 2),
               `2-3 years after diagnosis:censored (%)` = round((`censored_ac_treat_2_3`/`N_ac_treat_2_3`)*100, 2),
               `3-4 years after diagnosis:died in period (%)` = round((`died_ac_treat_3_4`/`N_ac_treat_3_4`)*100, 2),
               `3-4 years after diagnosis:censored (%)` = round((`censored_ac_treat_3_4`/`N_ac_treat_3_4`)*100, 2),
               `4-5 years after diagnosis:died in period (%)` = round((`died_ac_treat_4_5`/`N_ac_treat_4_5`)*100, 2),
               `4-5 years after diagnosis:censored (%)` = round((`censored_ac_treat_4_5`/`N_ac_treat_4_5`)*100, 2),
               `5-6 years after diagnosis:died in period (%)` = round((`died_ac_treat_5`/`N_ac_treat_5`)*100, 2),
               `5-6 years after diagnosis:censored (%)` = round((`censored_ac_treat_5`/`N_ac_treat_5`)*100, 2),
               `Initial treatment phase (0-1 years):moved to EOL (%)` = round((`censored_eol_treat_0_1`/`N_treat_0_1`)*100, 2),
               `Initial treatment phase (0-1 years):censored (%)` = round((`censored_end_treat_0_1`/`N_treat_0_1`)*100, 2),
               `Continuing phase (1-2 years):moved to EOL (%)` = round((`censored_eol_treat_1_2`/`N_treat_1_2`)*100, 2),
               `Continuing phase (1-2 years):censored (%)` = round((`censored_end_treat_1_2`/`N_treat_1_2`)*100, 2),
               `Continuing phase (2-3 years):moved to EOL (%)` = round((`censored_eol_treat_2_3`/`N_treat_2_3`)*100, 2),
               `Continuing phase (2-3 years):censored (%)` = round((`censored_end_treat_2_3`/`N_treat_2_3`)*100, 2),
               `Continuing phase (3-4 years):moved to EOL (%)` = round((`censored_eol_treat_3_4`/`N_treat_3_4`)*100, 2),
               `Continuing phase (3-4 years):censored (%)` = round((`censored_end_treat_3_4`/`N_treat_3_4`)*100, 2),
               `Continuing phase (4-5 years):moved to EOL (%)` = round((`censored_eol_treat_4_5`/`N_treat_4_5`)*100, 2),
               `Continuing phase (4-5 years):censored (%)` = round((`censored_end_treat_4_5`/`N_treat_4_5`)*100, 2),
               `Continuing phase (5-6 years):moved to EOL (%)` = round((`censored_eol_treat_5`/`N_treat_5`)*100, 2),
               `Continuing phase (5-6 years):censored (%)` = round((`censored_end_treat_5`/`N_treat_5`)*100, 2),
               `End of life phase:<12 months in phase (%)` = round((`censored_died_eol`/`N_eol`)*100, 2),
               `End of life phase:12 months in phase (%)` = round((`censored_end_eol`/`N_eol`)*100, 2)) %>%
  select(`Cancer type`, `Stage`, `0-1 years after diagnosis:died in period (%)`:`End of life phase:12 months in phase (%)`)
...
# Add on cenorsing
df_allcause_annual_and_phase_personsNmonthscensored_8cancers <- left_join(df_allcause_annual_and_phase_personsNmonths_8cancers, df_allcause_annual_and_phase_Ncensored_8cancers, by = c("Cancer type" ,"Stage")) %>%
  dplyr::rename_with(~stringr::str_replace(.,pattern = ' mean proportion',replacement =':mean follow-up observed (%)'),
...
df_allcause_unadj_total_costs <- df_allcause_unadj_total_costs_basic %>%
  ...
  mutate(across(where(is.character), ~ gsub("([[:digit:]])[.]([[:digit:]])",
       "\\1·\\2", .))) 
...
select(cancer_type, cost_outcome, variable, estimate, conf.low, conf.high) %>%
  ...
  filter(`CI: 2.5%` != 0) %>%
  mutate(across(`Average marginal effect`:`CI: 97.5%`, \(x) round(x, 2))) %>%
...
df_net_unadj_annual_costs <- df_net_unadj_annual_costs %>%
  ...
  mutate(Characteristic = ifelse(duplicated(Characteristic), NA, Characteristic)) %>%
  ungroup()%>%
  mutate(across(where(is.character), ~ gsub("([[:digit:]])[.]([[:digit:]])","\\1·\\2", .)))
...
df_net_regression_absolute <- df_net_AME_basic %>% 
  ...
  mutate(absolute_average = estimate + stage_1_estimate,
         estimate_SE = (conf.high - conf.low) / (2*qnorm(0.975)),
         combined_SE = sqrt( ((stage_1_std.error)^2)  + ((estimate_SE)^2) ),
         absolute_conf_low = absolute_average - qnorm(0.975) * combined_SE,
         absolute_conf_high = absolute_average + qnorm(0.975) * combined_SE) %>%
...
df_net_regression_absolute_monthly <- df_net_regression_absolute %>%
  ...
  rename("0-6 months before diagnosis" = "0-6 months prior to diagnosis mean proportion",
    ...
    mutate(Stage = case_when(Stage == "1" ~ "I",
        ...
        mutate(`Monthly mean cost` =  round(`absolute_average`/(12 *`Mean proportion of phase`), 1),
               `Monthly 2.5CI` =   round(absolute_conf_low/(12 *`Mean proportion of phase`), 1),
               `Monthly 97.5CI` =  round(absolute_conf_high /(12 *`Mean proportion of phase`), 1)) %>%
  ...
  mutate(across(where(is.numeric), ~ gsub("([[:digit:]])[.]([[:digit:]])","\\1·\\2", .)))
...
df_prediag_models_results <- res_path_prediag_models %>% 
  ...
  pivot_wider(names_from = `Regression model`, values_from = c(estimate, std.error), names_glue = "{`Regression model`}: {.value}")%>%
     mutate(across(where(is.numeric), ~ gsub("([[:digit:]])[.]([[:digit:]])",
       "\\1·\\2", .))) 
...
write_csv(x = l_allcause_descriptives_8cancers[[1]] ,
  file = "~/Miscellaneous/HDI/Code for publication/table_1.csv")
...
write.csv(x = df_net_regression_stage1_table_format %>% 
  ...
  filter(Covariate %in% c("Stage: I", "Stage: II", "Stage: III", "Stage: IV")) %>%
  pivot_longer(cols = "Colorectal":"Pancreatic", names_to = "Cancer type", values_to = "Cost") %>%
  pivot_wider(names_from = "Covariate", values_from = "Cost"),
  file = "~/Miscellaneous/HDI/Code for publication/table_2.csv")
...
gt(l_allcause_descriptives_8cancers[[3]]) |> sub_missing( missing_text = "") |> cols_label(`...1` = " ", `...2` = " " ) |>
    tab_spanner(label = "Stage at diagnosis (n [%])", columns = c( `1`, `2`, `3`, `4`, `Unknown/Missing`)) |>
  cols_align(align = "right", columns = c( `1`, `2`, `3`, `4`, `Unknown/Missing`))|>
  tab_source_note(source_note = "GP: general practitioner; SD: standard deviation; TWW: GP urgent cancer two-week wait referral.")
...
gt(df_stage_missing %>% 
  ...
  sub_missing( missing_text = "") |> cols_label(`site` = "Cancer type", `diagnosis year` = "Year of diagnosis" ) |>
    tab_spanner(label = "Stage at diagnosis (n [%])", columns = c( `1`, `2`, `3`, `4`, `Unknown/Missing`))|>
  cols_align(align = "right", columns = c(`diagnosis year`, `1`, `2`, `3`, `4`, `Unknown/Missing`))
```
...
gt(df_net_regression_stage1_table_format %>% 
  ...
  tab_options(row_group.default_label = "Average marginal effect (95% CI): covariates, potential confounders included in regression") |>
  ...
  tab_source_note(source_note = "Age was considered a continuous covariate, giving the average marginal effect per additional year of age. All other covariates were considered binary variables with reference categories: Stage: I; Gender: Male; CCI score: 0; Ethnicity: White; IMD quintile: 1 (least deprived)")
...
gt(df_allcause_annual_and_phase_personsNmonthscensored_8cancers %>%
     mutate(across(`Diag phase:mean follow-up observed (%)`:`End of life phase:mean follow-up observed (%)`, ~ .x * 100)) %>%
          select(`Cancer type`, Stage,  `Diag phase:mean follow-up observed (%)`:`End of life phase:mean follow-up observed (%)`,
                 `Initial treatment phase (0-1 years):moved to EOL (%)`:`End of life phase:12 months in phase (%)` ) %>%
     select(`Cancer type`, Stage, contains("Diag phase"), contains("Initial treatment phase (0-1 years)"),
            contains("Continuing phase (1-2 years)"), contains("Continuing phase (2-3 years)"),
            contains("Continuing phase (3-4 years)"), contains("Continuing phase (4-5 years)"),
            contains("Continuing phase (5-6 years)"), contains("End of life")) %>%
     select(-`Diag phase:mean follow-up observed (%)`,
            -`Initial treatment phase (0-1 years):censored (%)`,
            -`Continuing phase (1-2 years):censored (%)`) %>%
     mutate(`Cancer type` = ifelse(duplicated(`Cancer type`), NA, `Cancer type`)) %>%
     mutate(across(where(is.numeric),  round, 0)) %>%
     mutate(across(where(is.numeric), ~ gsub("([[:digit:]])[.]([[:digit:]])",
       "\\1·\\2", .)))) |>
  sub_missing( missing_text = "")|>
  tab_spanner_delim(delim = ":", reverse = FALSE) %>%
  tab_source_note(source_note = "Censoring occurs once the end of the follow-up period is reached (31 Dec 2019)") 
...
gt(df_allcause_annual_and_phase_personsNmonthscensored_8cancers %>%
     mutate(across(`0-6 months prior to diagnosis:mean follow-up observed (%)`:`5-6 years after diagnosis:mean follow-up observed (%)`, ~ .x * 100)) %>%
          select(`Cancer type`, Stage,  `0-6 months prior to diagnosis:mean follow-up observed (%)`:`5-6 years after diagnosis:mean follow-up observed (%)`,
                 `0-1 years after diagnosis:died in period (%)`:`5-6 years after diagnosis:censored (%)` ) %>%
     select(`Cancer type`, Stage, contains("0-6"), contains("0-1 years"),
            contains("1-2 years"), contains("2-3 years"),
            contains("3-4 years"), contains("4-5 years"),
            contains("5-6 years")) %>%
     select(-`0-6 months prior to diagnosis:mean follow-up observed (%)`,
            -`0-1 years after diagnosis:censored (%)`,
            -`1-2 years after diagnosis:censored (%)`) %>%
     mutate(`Cancer type` = ifelse(duplicated(`Cancer type`), NA, `Cancer type`)) %>%
     mutate(across(where(is.numeric),  round, 0)) %>%
     mutate(across(where(is.numeric), ~ gsub("([[:digit:]])[.]([[:digit:]])",
       "\\1·\\2", .)))) |>
...
## S5: baseline costs results

### Supplemental Table S5a: regression summaries for the baseline costs prediction models.
...
### Supplemental Table S5b: average baseline costs by year of diagnosis: Colorectal cancer.
...
### Supplemental Table S5c: average baseline costs by year of diagnosis: head and neck cancer.
...
### Supplemental Table S5d: average baseline costs by year of diagnosis: liver cancer.
...
### Supplemental Table S5e: average baseline costs by year of diagnosis: lung cancer.
...
### Supplemental Table S5f: average baseline costs by year of diagnosis: lymphoma cancer.
...
### Supplemental Table S5g: average baseline costs by year of diagnosis: oesophageal cancer.
...
### Supplemental Table S5h: average baseline costs by year of diagnosis: ovarian cancer.
...
### Supplemental Table S5i: average baseline costs by year of diagnosis: pancreatic cancer.
...
### S6: monthly hospital costs results

### Supplemental Table S6a: monthly net regression-adjusted hospital costs
...
df_N_cancertypestage <- l_net_descriptives_8cancers[[1]] %>%
  fill(`...1`, .direction = "down") %>%
  filter(`...1` == "Stage at diagnosis") %>%
  filter(`...2` != "Unknown/Missing") %>% 
  rename(Stage = `...2`) %>%
  select(-`...1`) %>%
  pivot_longer(`Lung N (%)`:`Pancreas N (%)`, names_to = "cancer_type", values_to = "N") %>%
  mutate(Stage = case_when(Stage == "1" ~ "I",
                           Stage == "2" ~ "II",
                           Stage == "3" ~ "III",
                           Stage == "4" ~ "IV", .default = NA)) %>%
  mutate(cancer_type = case_when(grepl(c("Lung"), cancer_type) ~ "Lung",
                                 grepl(c("Colorectal"), cancer_type) ~ "Colorectal",
                                 grepl(c("Head_neck"), cancer_type) ~ "Head and neck",
                                 grepl(c("Liver"), cancer_type) ~ "Liver",
                                 grepl(c("Lymphoma"), cancer_type) ~ "Lymphoma",
                                 grepl(c("Oesophagus"), cancer_type) ~ "Oesophageal",
                                 grepl(c("Ovary"), cancer_type) ~ "Ovarian",
                                  grepl(c("Pancreas"), cancer_type) ~ "Pancreatic", .default = NA)) %>%
  mutate(N = as.numeric(stringr::str_replace(N, "(?s) .*", "") )) %>%
  mutate(N = format(N , nsmall=0, big.mark=","))


Figure_1_net_overall_cost <- df_net_regression_absolute %>%
        filter(cost_outcome == "Overall costs") %>%
       filter(Stage != "Missing/Unknown") %>%
    left_join(df_N_cancertypestage, by = c("cancer_type", "Stage")) %>%
    mutate(N = paste0("n = ", N)) %>%
        ggplot() +
        ...
        scale_y_continuous(limits = c(-10000,50000), breaks = c(0, 10000, 20000, 30000, 40000, 50000),labels = scales::label_currency(scale = .001, prefix = "£", suffix = "K")) +
    ...
    panel.grid.minor = element_blank()
        ...
        geom_text(mapping = aes(x = Stage, y = -5000, label = N ), size= 3) 

# Add roboto font for Helen
# library(sysfonts)
# library(showtext)
...
Helen_Figure_1_net_overall_cost <- df_net_regression_absolute %>%
        filter(cost_outcome == "Overall costs") %>%
       filter(Stage != "Missing/Unknown") %>%
    left_join(df_N_cancertypestage, by = c("cancer_type", "Stage")) %>%
    filter(cancer_type %in% c("Colorectal", "Head and neck",  "Lymphoma", "Ovarian")) %>%
    #  filter(cancer_type %in% c("Liver", "Lung",  "Oesophageal", "Pancreatic")) %>%
 # mutate(cancer_type = factor(cancer_type, levels = c("Colorectal", "Head and neck", "Lymphoma", "Ovarian", "Liver", "Lung", "Oesophageal", "Pancreatic"))) %>%
  # mutate(cancer_type = factor(cancer_type, levels = c("Colorectal", "Liver", "Head and neck","Liver",  "Lymphoma","Oesophageal", "Ovarian", "Pancreatic"))) %>%
    mutate(N = paste0("n = ", N)) %>%
        ggplot() +
        geom_bar(aes(y = absolute_average, x = Stage, group = Stage, fill = Stage),
                 position= position_dodge(0.5),
...
ggsave(paste0("~/Miscellaneous/HDI/Code for publication/", "Landscape_goodprog_fig_1.png"), Helen_Figure_1_net_overall_cost, device = "png",
       width = 300,
       height = 100,
       units = "mm")
...
ggsave(paste0("~/Miscellaneous/HDI/Code for publication/", "fig_1.eps"), Figure_1_net_overall_cost, device = "eps",
       width = 200,
       height = 200,
       units = "mm")
...
Figure_2_net_PoC_cost <- df_net_regression_absolute %>%
  ...
  mutate(cost_outcome = factor(cost_outcome, levels = c( "Diagnosis", "Initial\n(0-1y)", "Continuing\n(1-2y)", 
       ...
       filter(Stage != "Missing/Unknown") %>%
        ...
        coord_flip() +
        scale_y_continuous(labels = scales::label_currency(scale = .001, prefix = "£", suffix = "K"), breaks = c(0,30000), limits = c(-10000,40000)) +
        ...
        theme(legend.position="bottom",
              ...
              strip.text.y.left = element_text(face = "bold",angle = 0, size = 10),
               strip.text.x = element_text(face = "bold", angle = 0, size = 10),
...
ggsave(paste0("~/Miscellaneous/HDI/Code for publication/", "fig_2.eps"), Figure_2_net_PoC_cost, device = "eps",
       width = 200,
       height = 170,
       units = "mm")

## Annual period cost equivalent graph

Figure_X_net_annual_cost <- df_net_regression_absolute %>%
  filter(cost_outcome %in% c(  "0-1 years after diagnosis", "1-2 years after diagnosis",
                              "2-3 years after diagnosis", "3-4 years after diagnosis", "4-5 years after diagnosis")) %>%
  mutate(cost_outcome = factor(cost_outcome, levels = c(  "0-1 years after diagnosis", "1-2 years after diagnosis",
                              "2-3 years after diagnosis", "3-4 years after diagnosis", "4-5 years after diagnosis"))) %>%
       filter(Stage != "Missing/Unknown") %>%
          ggplot() +
        geom_bar(aes(y = absolute_average, x = fct_rev(Stage), group = fct_rev(Stage), fill = fct_rev(Stage)),
...
ggsave(paste0("~/Miscellaneous/HDI/Cost generalisation/", "annual.png"), Figure_X_net_annual_cost, device = "png",
       width = 200,
       height = 250,
       units = "mm")
...
Figure_3_net_PoC_cost_monthly <- df_net_regression_absolute_monthly %>%
  mutate(across(where(is.character), ~ gsub("([[:digit:]])[·]([[:digit:]])","\\1.\\2", .))) %>%
  mutate(across(absolute_average:`Monthly 97.5CI`, as.numeric)) %>%
...
ggsave(paste0("~/Miscellaneous/HDI/Code for publication/", "fig_3.eps"), Figure_3_net_PoC_cost_monthly, device = "eps",
       width = 170,
       height = 150,
       units = "mm")
...
HelenFigure_3_net_PoC_cost_monthly <- df_net_regression_absolute_monthly %>%
  mutate(across(where(is.character), ~ gsub("([[:digit:]])[·]([[:digit:]])","\\1.\\2", .))) %>%
  mutate(across(absolute_average:`Monthly 97.5CI`, as.numeric)) %>%
  filter(cost_outcome %in% c("0-1 years after diagnosis")) %>%
    mutate(cost_outcome = case_when(cost_outcome == "0-1 years after diagnosis" ~ "Annual cost: 0-1 years after diagnosis", .default = cost_outcome)) %>%
  mutate(cost_outcome = factor(cost_outcome, levels = c("Annual cost: 0-1 years after diagnosis"))) %>%
  ggplot() +
   geom_bar(aes(y = `Monthly mean cost`, x = cancer_type, group = fct_rev(Stage), fill = fct_rev(Stage)),
                 position= position_dodge(0.5),
                 stat = "identity",
  ...
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
        ...
        theme(legend.position="bottom",
              ...
              strip.text = element_text( face = "bold", size = 13),
              axis.text.x = element_text(face = "bold", angle = 0, size = 13),
              axis.text.y = element_text(angle = 0, size = 13) )  

ggsave(paste0("~/Miscellaneous/HDI/Code for publication/", "fig_3_Helen.png"), HelenFigure_3_net_PoC_cost_monthly, device = "png",
       width = 300,
       height = 150,
       units = "mm")
